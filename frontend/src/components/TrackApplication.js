...
const TrackApplication = () => {
  ...
  useEffect(() => {
    const fetchPaymentStatus = async () => {
      try {
        const response = await fetch(`http://13.60.205.99:5000/api/payments/${applicationId}`);
        if (response.ok) {
  ...
  const handleTrackApplication = async (e) => {
    ...
    try {
      const response = await fetch(`http://13.60.205.99:5000/api/applications/${applicationId}`);
      if (response.ok) {
        const data = await response.json();
